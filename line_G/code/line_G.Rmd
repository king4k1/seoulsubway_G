...
ridership_mean_infoset_train <- ridership_mean_infoset %>% 
  ...
  left_join(station_seoul_info) %>% ungroup %>% 
  mutate(Time = as.numeric(Time), Day = as.factor(Day), on_off = as.factor(on_off)) %>% 
  dplyr::select(-station, -address) %>%  data.frame()
...
load(file="../data/fit_gbm_0708.RData")
...
predict_count_gbm <- function(day_choose = "Fri", time_choose = "20", on_off_choose = "off"){
  ...
  new_data <- station_seoul_info %>% 
    ...
    dplyr::select(colnames(ridership_mean_infoset_train)[c(-3,-5)])
...
```{r}
on_fri_15 <- predict_count_gbm(day_choose = "Fri", time_choose = "15", on_off_choose = "on")
off_fri_15 <- predict_count_gbm(day_choose = "Fri", time_choose = "15", on_off_choose = "off")
...
MC_sampling <- function(input_data, on_off_choose){
  ...
  input_data <- input_data %>% mutate(count = ifelse(count<0, 0, count))
  for(i in 1:length(nms_get)){
    nrow_i <- input_data %>% filter(station==nms_get[i]) %>%
      dplyr::select(count) %>% t() %>% as.numeric() %>% ceiling()
    if(nrow_i > 0){
      ...
      tot_except_i <- sum(input_data$count[input_data$station != nms_get[i]] %>%
                            ceiling())
      generate <- sample(x = input_data$station[input_data$station != nms_get[i]],
                         replace =TRUE, 
                         size = input_data$count[input_data$station == nms_get[i]] %>%
                         ceiling(), 
                         prob = input_data$count[input_data$station != nms_get[i]] %>%
                         ceiling()/tot_except_i)
...
on_Sun_0 <- predict_count_gbm(day_choose = "Sun", time_choose = "7", on_off_choose = "on")
on_Sun_0 %>% MC_sampling("on") %>% filter(depart == "국민대") %>% 
...
summarise_count_db <- function(day_choose, time_choose){
  ...
  on_case <- predict_count_gbm(day_choose = day_choose, 
                               time_choose = time_choose, 
                               on_off_choose = "on") %>% 
    MC_sampling(on_off_choose =  "on") %>% 
    dplyr::select(day, depart, arrival, time, count) %>%
    mutate(set = paste0(depart, "-", arrival)) %>% 
    rename(ride_on = depart, ride_off = arrival)
  off_case <- predict_count_gbm(day_choose = day_choose, 
                                time_choose = time_choose, 
                                on_off_choose = "off") %>%
...
route_tot <- function(day_choose, time_choose){
  route_choose <- as.data.table(subway_04_route_daymean)
  setkey(route_choose, day)
  route_choose_frist <- route_choose[day==day_choose,]
  setkey(route_choose_frist, time)
...
predict_demand_line <- function(day_choose, time_choose, line_choose) {
  ...
  pred_on <- predict_count_gbm(day_choose = day_choose, 
                               time_choose = time_choose, on_off_choose = "on")
  pred_off <- predict_count_gbm(day_choose = day_choose,
                                time_choose = time_choose, on_off_choose = "off")
  result_sum <- summarise_count_db(day_choose = day_choose, time_choose = time_choose)
  result_onoff <- pred_on %>% rename(pred_on = count, on = on_off) %>% 
    left_join(pred_off) %>% rename(pred_off = count, off = on_off) %>% 
...
predict_demand_line(day_choose="Sun", time_choose="7", line_choose="G")
```
  ...
  for(j in seq_along(time_choose_list)){
      ...
      route_total[[paste0(day_choose_list[i], "-", time_choose_list[j])]] <-
        route_tot(day_choose=day_choose_list[i],
                  time_choose=time_choose_list[j]) %>% nrow()
  }
}
...
route_choose_second %>% 
  head() %>% kable("latex")
route_tot(day_choose=day_choose_list[i], time_choose=time_choose_list[j]) %>%
  head() %>% kable("latex")
...
route_tot(day_choose="Fri", time_choose="15")

#save(file="../data/predict_week.RData", predict_week_onoff, predict_week_path)
```

```{r}
load(file="../data/predict_week.RData")
```
...
onoff_daily_gather %>%
  ...
  ylab("평균 이용객 수") + xlab("요일") + 
  theme(strip.text = element_text(size=5), axis.title=element_text(size=7),
        legend.position = "none",
        axis.text = element_text(size=6))
#ggsave(filename="on_off_lineG.pdf", path="D:/workspace/seoulsubway_G/line_G/figure")
...
predict_week_onoff_gather %>% 
  ...
  geom_line() + facet_grid(~station) +
  coord_cartesian(ylim=c(0, 1600)) +  theme_bw()  +
  ylab("평균 이용객 수") + xlab("승차 시간") + labs(col="요일") +
  theme(strip.text = element_text(size=5), axis.title=element_text(size=7),
        axis.text = element_text(size=6), legend.title = element_text(size=7),
        legend.text = element_text(size=7),
        legend.position = "bottom") + scale_fill_viridis_c() +
  scale_color_viridis_d() 
# ggsave(filename="on_time_lineG.pdf", path="D:/workspace/seoulsubway_G/line_G/figure")
...
predict_week_onoff_gather %>% 
  ...
  ggplot(aes(x=time, y= count, group=day, col=day)) + 
  geom_line() + facet_grid(~station) +
  coord_cartesian(ylim=c(0, 1500)) +  theme_bw()  +
  ylab("평균 이용객 수") + xlab("하차 시간") + labs(col="요일") +
  theme(strip.text = element_text(size=6), axis.title=element_text(size=7),
        axis.text = element_text(size=6), legend.title = element_text(size=7),
        legend.text = element_text(size=7),
        legend.position = "bottom") + scale_fill_viridis_c() +
  scale_color_viridis_d() 
#ggsave(filename="off_time_lineG.pdf", path="D:/workspace/seoulsubway_G/line_G/figure")
...
```{r, fig.align='center'}
onoff_lineG_new <- predict_week_onoff %>% 
...
predict_week_onoff %>% 
  ...
  summarise(ride_on = sum(pred_on),
            ride_off = sum(pred_off),
            total = (ride_on+ride_off)) %>% filter(! day %in% c("일", "토")) %>% 
  summarise(mean = mean(total)/2)
```
...
pred_sec_count <- predict_week_path %>% 
...
pred_sec_count %>% 
  group_by(day, type) %>% summarise(mean_tot = mean(total) %>% ceiling) %>%
  ungroup %>% rename(요일=day) %>% 
  spread("type","mean_tot") %>% kable("latex")

inner_G <- pred_sec_count$section %>% unique()
...
transfer_lineG %>% kable("latex")

transfer_lineG$day <- c("금", "목", "수", "월", "일", "토", "화")
tot_onoff_result <- merge(transfer_lineG, onoff_lineG_new) %>%
  mutate(on = on_count + ride_on,
         off = off_count + ride_off,
         tot = on + off)
tot_onoff_result$tot[c(-5, -6)] %>% mean()
...
summarise_count_G <- function(day_choose, time_choose){
  route_choose <- as.data.table(subway_04_route_daymean)
  setkey(route_choose, day)
  route_choose_frist <- route_choose[day==day_choose,]
  setkey(route_choose_frist, time)
...
for(i in seq_along(day_choose_list)){
  for(j in 1:length(time_choose_list)){
    tot_day_list[[paste0(day_choose_list[i], "-", time_choose_list[j])]] <-
...
for(k in 1:19){
  daily_tot[k,1] <- tot_day_list[[k]]$tot_sum
  daily_tot[k,2] <- tot_day_list[[(k+19)]]$tot_sum
  daily_tot[k,3] <- tot_day_list[[(k+38)]]$tot_sum
  daily_tot[k,4] <- tot_day_list[[(k+57)]]$tot_sum
  daily_tot[k,5] <- tot_day_list[[(k+76)]]$tot_sum
  daily_tot[k,6] <- tot_day_list[[(k+95)]]$tot_sum
  daily_tot[k,7] <- tot_day_list[[(k+114)]]$tot_sum
}

inner_result <- daily_tot %>% mutate_all(as.numeric) %>% colSums()
data.frame(day = daily_tot %>% names(), inner_count = inner_result) %>% 
  left_join(tot_onoff_result) %>% mutate(final = tot + inner_count)
...
ggmap(get_map(location = c(126.95, 37.58), zoom = 12)) + 
   geom_path(data=from_to_path_set_tot, 
             aes(x=long, y=lat, col=count, group=set), size=2) +
  scale_x_continuous(breaks = c(126.8, 126.9, 127)) +
  scale_y_continuous(breaks = c(37.5, 37.6)) +
  facet_grid(time2~type) +
  labs(col="평균 이용객 수") + xlab("경도") + ylab("위도") +
  scale_fill_viridis_c() + scale_color_viridis_c() +
  theme(axis.text.x = element_text(angle = 90))

#ggsave(filename="section_demand.pdf", path="D:/workspace/seoulsubway_G/line_G/figure")
```

```{r}
predict_week_path_longlat_key %>% 
  filter(type == "상행") %>% 
  mutate(day = factor(day, levels=c("Sun","Mon","Tue","Wed","Thr","Fri","Sat"),
                      labels=c("일", "월","화", "수", "목", "금", "토"))) %>% 
  ...
  ylab("이용객 수") + xlab("시간") +
  theme(strip.text = element_text(size=5), axis.title=element_text(size=7),
        axis.text = element_text(size=5), legend.title = element_text(size=6),
        legend.text = element_text(size=6), legend.key.size = unit(0.5, "cm"),
        legend.position = "bottom") + 
  scale_fill_viridis_c() + scale_color_viridis_d()
#ggsave(filename="up_section.pdf", path="D:/workspace/seoulsubway_G/line_G/figure")
...
predict_week_path_longlat_key %>% 
  ...
  theme(strip.text = element_text(size=5), axis.title=element_text(size=7),
        ...
        legend.position = "bottom") + 
  scale_fill_viridis_c() + scale_color_viridis_d()
#ggsave(filename="down_section.pdf", path="D:/workspace/seoulsubway_G/line_G/figure")

#ggsave(filename="up_down_section.pdf", path="D:/workspace/seoulsubway_G/line_G/figure")
```
