...
1. 도로주소 시스템에서 제공되는 shape파일을 이용하여 두 지점 간 최단거리를 계산한다.

* 도로명주소 개발자센터 > 전자지도 다운로드 http://www.juso.go.kr/addrlink/main.do

두 역 사이 거리를 측정한 후, "서울메트로. "지하철 표정속도는 34km/h이다." [1] 을 참고하여 역간 소요시간을 계산하였다. 

2. 공공데이터포털 내 제공되는 `환승역, 환승거리 및 소요시간 정보(서울교통공사 17.10 기준)`를 참고하여 환승 시 소요시간을 반영하였다. 하지만 모든 환승역에 대한 정보가 제공되지 않아 제공되지 않는 환승역은 `이세중. “환승할 때 뛰지 마세요”…서울지하철 열차 시간표 조정 (KBS NEWS),2017년 6월 20일` [2] 을 참고하였다. 또한 일반적인 경우 환승 시 지하철 대기시간이 추가적으로 소요된다. 따라서 대기시간으로 2분을 추가하였다.

3. 지하철 정차시간을 고려하기 위하여 소요시간에 이동 역당 30초를 추가하였다.
...
shortestpath_plot(departure = "길음",  arrival = "청량리", google_key = API_key, zoom = 13)
```
...
seoul + 
  geom_path(aes(x=long, y=lat), col="brown", size =5, data=line_G) +
  geom_point(aes(x=long, y=lat, col=ifelse(Transfer==0, 'N', 'Y')), 
             size =5, data=line_G) + labs(col = "환승가능") +
  scale_color_manual(values=c("green", "red")) + 
  geom_label_repel(aes(x=long, y=lat, label=Name), fill = "black", col = "white", 
                 alpha = 0.7, size=5, data=line_G) +
  theme(strip.text = element_text(size=15), axis.title=element_text(size=15),
        legend.title = element_text(size=15), legend.text = element_text(size=15),
        axis.text = element_text(size=15)) + xlab("경도") + ylab("위도")
  
  #scale_y_continuous(limits = c(37.515, 37.63), expand = c(0, 0))
```
...
```{r}
shortestpath_G_plot <- function (departure, arrival, google_key, zoom = zoom){
    ...
    g <- ggmap(get_map(c(meanX, meanY), zoom = zoom, maptype = "toner-lite", 
        ...
        y = as.numeric(lat), col = Line)) + geom_label(data = Path %>% 
...
#### 5.1.2 [국가공간정보포털 내 인구주택총조사 통계_인구부문(2016)](http://data.nsdi.go.kr/dataset/20171206ds00009/resource/48c14840-4aca-4779-aed0-6e218404509f?inner_span=True)
...
#### 5.1.3 [국가공간정보포털 내 인구주택총조사 통계_사업체부문(2016)](http://data.nsdi.go.kr/dataset/20171206ds00006/resource/4f363f8b-d94b-4c66-a633-b5ea1989bf9d)
...
#### 5.1.4 [국가공간정보포털 내 인구주택총조사 통계_가구부분(2016)](http://data.nsdi.go.kr/dataset/20171206ds00007/resource/88154502-eaeb-4fed-b82c-359d3ad0ed27)
...
#### 5.1.5 [다음API](https://developers.kakao.com/docs/restapi/tool)를 이용하여 6가지 변수 선정(`대형마트`, `학교`, `학원`, `문화시설`, `관광명소`, `병원`)
...
#### 5.1.6 버스정류장 위치데이터 이용 반경 500m 이내 버스정류장 수
...
1> 302개 지하철역을 대상으로 41개의 주위정보 데이터
  ...
  * 5가지 용도지역 비율 : usz_sang, usz_ju, usz_gong, usz_etc
...
1. `predict_count_rf(day_choose, time_choose, on_off_choose)` 
...
ridership_mean_infoset_train <- ridership_mean_infoset %>% 
  ungroup %>% dplyr::select(-station, -address) %>% 
...
rpart_predict <- station_seoul_info %>% 
  filter(station %in% station_predict) %>% 
  mutate(pred = predict(rpart_fit, new_data))
 
rf_predict <- station_seoul_info %>% 
  filter(station %in% station_predict) %>% 
  mutate(pred = predict(rf_fit, new_data))

plot(rpart_predict$pred, rf_predict$pred)
```

```{r}
predict_count_rf <- function(day_choose = "Fri", time_choose = "20", on_off_choose = "off"){
  ...
  station_seoul_info %>% 
    filter(station %in% station_predict) %>% 
    mutate(mean_count = predict(rf_fit, new_data)) %>% 
...
predict_count_rf(day_choose = "Fri", time_choose = "20", on_off_choose = "off")
...
```{r}
on_fri_15 <- predict_count_rf(day_choose = "Fri", time_choose = "15", on_off_choose = "on")
off_fri_15 <- predict_count_rf(day_choose = "Fri", time_choose = "15", on_off_choose = "off")
...
summarise_count_db <- function(day_choose, time_choose){
  ...
  on_case <- predict_count_rf(day_choose = day_choose, time_choose = time_choose, 
                              on_off_choose = "on") %>% MC_sampling( on_off_choose =  "on") %>% 
    dplyr::select(day, depart, arrival, time, count) %>% mutate(set = paste0(depart, "-", arrival)) %>% 
    rename(ride_on = depart, ride_off = arrival, mean_count = count)
  off_case <- predict_count_rf(day_choose = day_choose, time_choose = time_choose, 
                              on_off_choose = "off") %>% MC_sampling( on_off_choose =  "off") %>% 
    dplyr::select(day, depart, arrival, time, count) %>% mutate(set = paste0(depart, "-", arrival)) %>% 
    rename(ride_on = depart, ride_off = arrival, mean_count = count)
  unit <- route_choose_second %>% rbind(on_case, off_case) %>% 
    ...
    summarise(mean = sum(as.numeric(mean_count))) %>% as_tibble()
  unit
}
...
predict_demand_line <- function(day_choose, time_choose, line_choose) {
  ...
  pred_on <- predict_count_rf(day_choose = day_choose, time_choose = time_choose, on_off_choose = "on")
  pred_off <- predict_count_rf(day_choose = day_choose, time_choose = time_choose, on_off_choose = "off")
  ...
  result_onoff <- pred_on %>% rename(pred_on = mean_count, on = on_off) %>% 
    ...
    mutate(day = day_choose, pred_on = floor(pred_on),
           pred_off = floor(pred_off)) %>% 
...
#### 6.1 수요예측값을 이용한 탐색적 자료분석

앞단계에서의 예측값을 이용하여 우선 다음 2개의 데이터셋을 구축하였다.

```{r, eval=FALSE}
day_choose_list <- c("Sun", "Mon", "Tue", "Wed","Thr", "Fri", "Sat")
time_choose_list <- as.numeric(unique(ridership_mean_infoset$Time))
...
```{r out.width="75%", fig.align="center", echo=TRUE}
...
```{r out.width="75%", fig.align="center", echo=TRUE}
path_count_G(time_choose = "7", day_choose = "Mon", up_down="up")

path_count_G(time_choose = "7", day_choose = "Mon", up_down="down")
```
